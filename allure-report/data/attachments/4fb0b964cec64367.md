...
- Location: tests\cart_management\cart_management.spec.js:145:1
...
```ts
  ...
  156 | 
  157 |   // Wait for cart page to load first
  158 |   await page.waitForURL(/cart/, { timeout: 10000 });
  159 | 
  160 |   const increaseBtn = page.getByRole("button", { name: "Increase quantity of item" });
> 161 |   await increaseBtn.waitFor({ state: "visible", timeout: 10000 });
      |                     ^ Error: locator.waitFor: Test timeout of 30000ms exceeded.
  162 |   await increaseBtn.click();
  163 | 
  164 |   const decreaseBtn = page.getByRole("button", { name: "Reduce quantity of item" });
  165 |   await decreaseBtn.waitFor({ state: "visible", timeout: 10000 });
  166 |   await decreaseBtn.click({ force: true });
  167 | });
  168 | 
  169 |   test("CT_07 - Remove Product Button Works", async ({ page }) => {
  170 |     await page.goto("https://www.razer.com/store");
  171 | 
  172 |     await page
  173 |       .getByRole("button", { name: "Accept All" })
  174 |       .waitFor({ state: "visible", timeout: 10000 });
  175 |     await page.getByRole("button", { name: "Accept All" }).click();
  176 | 
  177 |     await page
  178 |       .getByRole("group", { name: "slide 1 of 16" })
  179 |       .getByLabel("BUY - New Razer Blade 18, For US$")
  180 |       .waitFor({ state: "visible", timeout: 10000 });
  181 |     await page
  182 |       .getByRole("group", { name: "slide 1 of 16" })
  183 |       .getByLabel("BUY - New Razer Blade 18, For US$")
  184 |       .click();
  185 | 
  186 |     await page
  187 |       .getByRole("button", { name: "Add to Cart - Razer Blade 18" })
  188 |       .waitFor({ state: "visible", timeout: 10000 });
  189 |     await page
  190 |       .getByRole("button", { name: "Add to Cart - Razer Blade 18" })
  191 |       .click();
  192 | 
  193 |     const removeBtn = page.getByRole("button", {
  194 |       name: "Remove item from cart",
  195 |     });
  196 |     await removeBtn.waitFor({ state: "visible", timeout: 10000 });
  197 |     await removeBtn.click();
  198 |   });
  199 | 
  200 |   test("CT_08 - Empty Cart Validation", async ({ page }) => {
  201 |     await page.goto("https://www.razer.com/store");
  202 | 
  203 |     await page
  204 |       .getByRole("button", { name: "Accept All" })
  205 |       .waitFor({ state: "visible", timeout: 10000 });
  206 |     await page.getByRole("button", { name: "Accept All" }).click();
  207 | 
  208 |     await page
  209 |       .getByRole("button", { name: "items in cart" })
  210 |       .waitFor({ state: "visible", timeout: 10000 });
  211 |     await page.getByRole("button", { name: "items in cart" }).click();
  212 | 
  213 |     await page
  214 |       .getByRole("link", { name: /Cart/i })
  215 |       .waitFor({ state: "visible", timeout: 10000 });
  216 |     await page.getByRole("link", { name: /Cart/i }).click();
  217 | 
  218 |     await expect(
  219 |       page.getByRole("heading", { name: /Your cart is empty/i }),
  220 |     ).toBeVisible({ timeout: 10000 });
  221 |   });
  222 | 
  223 |  test("CT_09 - Subtotal Validation", async ({ page }) => {
  224 |   await page.goto("https://www.razer.com/store");
  225 | 
  226 |   await page.getByRole("button", { name: "Accept All" }).waitFor({ state: "visible", timeout: 10000 });
  227 |   await page.getByRole("button", { name: "Accept All" }).click();
  228 | 
  229 |   await page.getByRole("group", { name: "slide 1 of 16" }).getByLabel("BUY - New Razer Blade 18, For US$").waitFor({ state: "visible", timeout: 10000 });
  230 |   await page.getByRole("group", { name: "slide 1 of 16" }).getByLabel("BUY - New Razer Blade 18, For US$").click();
  231 | 
  232 |   await page.getByRole("button", { name: "Add to Cart - Razer Blade 18" }).waitFor({ state: "visible", timeout: 10000 });
  233 |   await page.getByRole("button", { name: "Add to Cart - Razer Blade 18" }).click();
  234 | 
  235 |   await page.getByRole("link", { name: "Store", exact: true }).waitFor({ state: "visible", timeout: 10000 });
  236 |   await page.getByRole("link", { name: "Store", exact: true }).click();
  237 | 
  238 |   await page.getByRole("button", { name: "Close" }).waitFor({ state: "visible", timeout: 10000 });
  239 |   await page.getByRole("button", { name: "Close" }).click();
  240 | 
  241 |   await page.getByRole("group", { name: "slide 1 of 16" }).getByLabel("BUY - New Razer Blade 18, For US$").waitFor({ state: "visible", timeout: 10000 });
  242 |   await page.getByRole("group", { name: "slide 1 of 16" }).getByLabel("BUY - New Razer Blade 18, For US$").click();
  243 | 
  244 |   await page.getByRole("button", { name: "Add to Cart - Razer Blade 18" }).waitFor({ state: "visible", timeout: 10000 });
  245 |   await page.getByRole("button", { name: "Add to Cart - Razer Blade 18" }).click();
  246 | });
  247 | 
  248 |   test("CT_10 - Cart Persistence (refresh)", async ({ page }) => {
  249 |     await page.goto("https://www.razer.com/store");
  250 | 
  251 |     await page
  ...
  255 | 
  256 |     await page
  257 |       .getByRole("group", { name: "slide 1 of 16" })
  258 |       .getByLabel("BUY - New Razer Blade 18, For US$")
  259 |       .waitFor({ state: "visible", timeout: 10000 });
  260 |     await page
  261 |       .getByRole("group", { name: "slide 1 of 16" })
```