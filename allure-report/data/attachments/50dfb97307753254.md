...
```ts
  ...
  178 |     .click();
  179 | 
  180 |   // open details once (no repeated blind click)
  181 |   const detailsBtn = page.getByRole('button', { name: 'Show details' });
  182 | 
> 183 |   await expect(detailsBtn).toBeVisible();
      |                            ^ Error: expect(locator).toBeVisible() failed
  ...
  188 | 
  189 |   // verify item exists
  190 |   await expect(page.getByText(/Razer Blade 18/i)).toBeVisible();
  191 | 
  192 |   // verify details button still exists in cart context (if UI supports it)
  193 |   const cartDetailsBtn = page.getByRole('button', { name: 'Show details' });
  194 | 
  195 |   await expect(cartDetailsBtn).toBeVisible();
  196 | });
  197 | });
  198 | 
```