...
- Name: 06_storeFunctionalityu\store.spec.js >> Store Functionalities >> SF_03 - Fresh off line carousel navigation works
...
Locator: locator('#fresh-off-line').locator('.tile, .card').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('#fresh-off-line').locator('.tile, .card').first()
...
```ts
  ...
  38 |   });
  39 | 
  40 |   test("SF_03 - Fresh off line carousel navigation works", async ({ page }) => {
  41 | 
  42 |   const carousel = page.locator("#fresh-off-line");
  43 | 
  44 |   await expect(carousel).toBeVisible();
  45 | 
  46 |   const nextBtn = carousel.getByRole("button", { name: "Next" });
  47 |   const prevBtn = carousel.getByRole("button", { name: "Previous" });
  48 | 
  49 |   // capture initial state (first visible item)
  50 |   const initialCard = carousel.locator(".tile, .card").first();
> 51 |   await expect(initialCard).toBeVisible();
     |                             ^ Error: expect(locator).toBeVisible() failed
  52 | 
  53 |   // move forward
  54 |   await nextBtn.click();
  55 |   await nextBtn.click();
  56 | 
  57 |   // verify UI still active after movement
  58 |   await expect(carousel).toBeVisible();
  59 | 
  60 |   // move backward
  61 |   await prevBtn.click();
  62 | 
  63 |   // final sanity check
