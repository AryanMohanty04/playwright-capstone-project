...
- Name: 06_storeFunctionalityu\store.spec.js >> Store Functionalities >> CAROUSEL_01 - Fresh off line carousel navigation works
- Location: tests\06_storeFunctionalityu\store.spec.js:40:3
...
```ts
  ...
  38 |   });
  39 | 
  40 |   test("CAROUSEL_01 - Fresh off line carousel navigation works", async ({
  41 |     page,
  42 |   }) => {
  43 |     await page.goto("https://www.razer.com/");
  44 | 
> 45 |     await page.getByRole("button", { name: "Accept All" }).click();
     |                                                            ^ Error: locator.click: Test timeout of 30000ms exceeded.
  46 |     await page.getByRole("link", { name: "Store", exact: true }).click();
  47 | 
  48 |     const carousel = page.locator("#fresh-off-line");
  49 | 
  50 |     // initial state
  51 |     await expect(carousel).toBeVisible();
  52 | 
  53 |     const nextBtn = carousel.getByRole("button", { name: "Next" });
  54 |     const prevBtn = carousel.getByRole("button", { name: "Previous" });
  55 | 
  56 |     // move forward
  ...
  59 | 
  60 |     // move backward
  61 |     await prevBtn.click();
  62 | 
  63 |     // verify carousel still exists and usable
  64 |     await expect(nextBtn).toBeVisible();
  65 |     await expect(prevBtn).toBeVisible();
  66 |   });
  67 | });
  68 | 
```