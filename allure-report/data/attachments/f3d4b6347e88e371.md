...
- Name: 06_storeFunctionalityu\store.spec.js >> Store Functionalities >> SF_04 - Verify Product Image Navigation
- Location: tests\06_storeFunctionalityu\store.spec.js:63:3
...
```ts
  ...
  61 |   });
  62 | 
  63 |   test("SF_04 - Verify Product Image Navigation", async ({ page }) => {
  64 |     await page.goto("https://www.razer.com/");
  65 | 
> 66 |     await page.getByRole("button", { name: "Accept All" }).click();
     |                                                            ^ Error: locator.click: Test timeout of 30000ms exceeded.
  67 | 
  68 |     await page.getByRole("link", { name: "Store", exact: true }).click();
  69 | 
  70 |     // Click first product image
  71 |     await page.locator(".thumbnail-holder > img").first().click();
  72 | 
  73 |     // Close popup if it appears
  74 |     const closeButton = page.getByRole("button", { name: "Close" });
  75 | 
  76 |     if (await closeButton.isVisible()) {
  77 |       await closeButton.click();
  78 |     }
  79 | 
  80 |     // Verify redirected URL
  81 |     await expect(page).toHaveURL(/gaming-laptops\/razer-blade-18/);
  82 |   });
  83 | });
  84 | 
```