...
- Name: product_details\productDetails.spec.js >> Product Details >> test
- Location: tests\product_details\productDetails.spec.js:53:3
...
Call log:
  - waiting for locator('.usp-popup-header > svg').first()
...
```ts
  ...
  51 |   // });
  52 | 
  53 |   test('test', async ({ page }) => {
  54 |   await page.goto('https://www.razer.com/');
  55 | 
  56 |   await page.getByRole('button', { name: 'Accept All' }).click();
  57 | 
  58 |   const store = page.getByRole('link', { name: 'Store', exact: true });
  59 | 
  60 |   // hover instead of click
  61 |   await store.hover();
  62 | 
  63 |   // verify something from the hover menu is visible
  64 |   await expect(page.getByRole('link', { name: 'Laptops' })).toBeVisible();
  65 | 
> 66 |   await page.locator('.usp-popup-header > svg').first().click();
     |                                                         ^ Error: locator.click: Test timeout of 30000ms exceeded.
  67 |   await page.locator('.usp-popup-header > svg').first().click();
  68 | });
  69 | 
  70 | });
  71 | 
