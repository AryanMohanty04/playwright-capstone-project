...
```ts
  ...
  4  | dotenv.config();
  5  | 
  6  | test('Razer login test', async ({ page }) => {
  7  |   await page.goto('https://www.razer.com/');
  8  | 
  9  |   await page.getByRole('button', { name: 'Accept All' }).click();
> 10 |   await page.getByRole('link', { name: 'Account' }).click();
     |                                                     ^ Error: locator.click: Test timeout of 30000ms exceeded.
  11 | 
  12 |   await page.getByRole('textbox', { name: 'Email Address' })
  13 |     .fill(process.env.APP_EMAIL);
  14 | 
  15 |   await page.getByRole('textbox', { name: 'Password Show Password' })
  16 |     .fill(process.env.APP_PASSWORD);
  17 | 
  18 |   await page.getByRole('textbox', { name: 'Password Show Password' })
  19 |     .press('Enter');
  20 | 
  21 |   await expect(page).toHaveURL(/my-account/);
  22 | 
