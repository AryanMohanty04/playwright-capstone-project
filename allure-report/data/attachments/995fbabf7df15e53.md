...
- Location: tests\cart_management\cart_management.spec.js:116:1
...
```ts
  ...
  129 |     .click();
  130 | 
  131 |   const removeBtn = page.getByRole("button", {
  132 |     name: "Remove item from cart",
  133 |   });
  134 | 
  135 |   await expect(removeBtn).toBeVisible({ timeout: 10000 });
  136 |   await removeBtn.click();
  137 | 
  138 |   // 🔥 BEST ASSERTION
> 139 |   await expect(page.getByText(/Razer Blade 18/i)).toHaveCount(0);
      |                                                   ^ Error: expect(locator).toHaveCount(expected) failed
  140 | 
  141 |   // OR fallback:
  142 |   // await expect(page.getByText(/your cart is empty/i)).toBeVisible();
  143 | });
  144 |   test("CT_07 - Empty Cart Validation", async ({ page }) => {
  145 |     await page.goto("https://www.razer.com/store");
  146 | 
  ...
  153 | 
  154 |     await page
  155 |       .getByRole("button", { name: "Add to Cart - Razer Blade 18" })
  156 |       .click();
  157 | 
  158 |     await page.getByRole("button", { name: "Remove item from cart" }).click();
  159 |   });
  160 | 
  161 |   test("CT_08 - Empty Cart Validation", async ({ page }) => {
  162 |     await page.goto("https://www.razer.com/store");
  163 | 
  164 |     await page.getByRole("button", { name: "Accept All" }).click();
  165 | 
  166 |     await page.getByRole("button", { name: "items in cart" }).click();
  167 |     await page.getByRole("link", { name: /Cart/i }).click();
  168 | 
  169 |     await page.waitForURL(/cart/, { timeout: 10000 });
  170 | 
  171 |     await expect(
  172 |       page.getByRole("heading", { name: /your cart is empty/i }),
  173 |     ).toBeVisible();
  174 |   });
  175 | 
  176 |   test("CT_09 - Subtotal Validation", async ({ page }) => {
  177 |     await page.goto("https://www.razer.com/store");
  178 |     await page.getByRole("button", { name: "Accept All" }).click();
  179 |     await page
  180 |       .getByRole("group", { name: "slide 1 of 16" })
  181 |       .getByLabel("BUY - New Razer Blade 18, For US$")
  182 |       .click();
  183 |     await page
  184 |       .getByRole("button", { name: "Add to Cart - Razer Blade 18" })
  185 |       .click();
  186 |     await page.getByRole("button", { name: "items in cart" }).click();
  187 |     await page.getByRole("link", { name: /Cart/i }).click();
  188 |     await page.waitForURL(/cart/, { timeout: 10000 });
  189 |     await expect(page.getByText(/Subtotal/i)).toBeVisible();
  190 |   });
  191 | 
  192 |   test("CT_10 - Cart Persistence (refresh)", async ({ page }) => {
  193 |     await page.goto("https://www.razer.com/store");
  194 |     await page.getByRole("button", { name: "Accept All" }).click();
  195 |     await page
  196 |       .getByRole("group", { name: "slide 1 of 16" })
  197 |       .getByLabel("BUY - New Razer Blade 18, For US$")
  198 |       .click();
  199 |     await page
  200 |       .getByRole("button", { name: "Add to Cart - Razer Blade 18" })
  201 |       .click();
  202 |     await page.getByRole("button", { name: "Show details" }).click();
  203 |     await page.goto("https://www.razer.com/cart");
  204 |     await page.getByRole("button", { name: "Show details" }).click();
  205 |   });
  206 | 
  207 |   test("CT_11 - Increase quantity works up to limit", async ({ page }) => {
  208 |     await page.goto("https://www.razer.com/store");
  209 | 
  210 |     await page.getByRole("button", { name: "Accept All" }).click();
  211 | 
  212 |     // add product
  213 |     await page
  214 |       .getByRole("group", { name: "slide 1 of 16" })
  215 |       .getByLabel("BUY - New Razer Blade 18, For US$")
  216 |       .click();
  217 | 
  218 |     await page
  219 |       .getByRole("button", { name: "Add to Cart - Razer Blade 18" })
  220 |       .click();
  221 | 
  222 |     // increase quantity once (your current approach)
  223 |     const increaseBtn = page.getByRole("button", {
  224 |       name: "Increase quantity of item",
  225 |     });
  226 | 
  227 |     await increaseBtn.click();
  228 | 
  229 |     // optional safe check (NOT visibility-based)
  230 |     const qty = page.locator('input[type="number"]').first();
  231 | 
  232 |     // if input exists, validate it increased from 1 → 2
  233 |     if (await qty.count()) {
  234 |       const value = await qty.inputValue();
  235 |       expect(Number(value)).toBeGreaterThanOrEqual(2);
  236 |     }
  237 |   });
  238 | });
  239 | 
