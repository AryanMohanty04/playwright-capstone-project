...
test.describe("Homepage & Navigation Module", () => {
  test("HN_01 - Homepage loads successfully", async ({ page }) => {
    ...
    await expect(page).toHaveURL(/razer\.com/);
  });

  //Click on the popup cookies
  test('HN-02 Buy link redirects to product page', async ({ page }) => {
  await page.goto('https://www.razer.com/');
  await page.getByRole('button', { name: 'Accept All' }).click();
  await page.getByRole('link', { name: 'Buy' }).first().click();
  await expect(page).toHaveURL("https://www.razer.com/gaming-mice/razer-viper-v4-pro/RZ01-05630100-R3U1");
});

  //Clicking on the Buy Now Button
  test("HN_03 - Store navigation and homepage redirect works", async ({ page }) => {
    await page.goto("https://www.razer.com/");
    await page.getByRole("button", { name: "Accept All" }).click();
